...
class AdvertController extends AuthController
{
    ...
    public function actionFileUploadGeneral()
    {
        ...
        $path = Yii::getAlias('@frontend/web/upload/adverts/'.$id.'/general');
    ...
    public function actionFileUploadImages()
    {
        if (Yii::$app->request->isPost) {
            $id = Yii::$app->request->post('advert_id');
            $path = Yii::getAlias('@frontend/web/upload/adverts/'.$id);
            BaseFileHelper::createDirectory($path);
    ...
    public function actionStep2() {
        ...
        if($general_image = $model->general_image){
            $image[] =  '<img src="/upload/adverts/' . $model->id_advert . '/general/small_' . $general_image . '" width=250>';
        }
            ...
            if(is_dir($path)) {
                ...
                foreach ($files as $file) {
                    if (strstr($file, "small_") && !strstr($file, "general")) {
                        $images_add[] = '<img src="/upload/adverts/' . $model->id_advert . '/' . basename($file) . '" width=250>';
                    }
                }
            }
        }
